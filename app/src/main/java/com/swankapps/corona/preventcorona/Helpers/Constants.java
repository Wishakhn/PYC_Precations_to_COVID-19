package com.swankapps.corona.preventcorona.Helpers;

public class Constants {
    public static final String FRAG_TYPE_PARAM ="user_auth_as";
    public static final String PYC_LOG ="PYC-Logs";
    public static final String CHECK_INTERNET = "Please check your internet connection";
    public static final String SERVER_WRONG = "Server time out! please try again later";
    public static final String WRONG_MESSAGE = "Something went wrong";
    public static final String OPPS = "Opps !!";
    public static final String SORRY = "Sorry !!";
    public static final int RESULT_LOAD_IMAGE = 1001;
    public static final int RESULT_CAPTURE_IMAGE = 1002;
    public static final int REQUEST_PERMISSION_CODE = 2001;


    public  static  final  String URL_IMF_BLOG = "https://blogs.imf.org/2020/03/23/covid-19-pandemic-and-the-middle-east-and-central-asia-region-facing-dual-shock/";
    public  static  final  String URL_WHO_BLOG = "https://www.who.int/health-topics/coronavirus#tab=tab_1";
    public  static  final  String URL_UNICEF_BLOG = "https://www.unicef.org/pakistan/coronavirus-disease-covid-19-what-parents-should-know";

}
